...
const Register = () => {
    ...
    return (
        <Box sx={{ borderRadius: 2, boxShadow: 5, background: grey[300], p: 5, m: '10vh auto', width: { xs: '100%', sm: '80%', md: '50%', lg: '40%', xl: '30%' } }}>
            ...
            <TextField fullWidth placeholder='Enter your password' sx={{ mb: 1, bgcolor: 'white' }} />
            <TextField fullWidth placeholder='Confirm password' sx={{ bgcolor: 'white' }} />
            <Button variant='contained' fullWidth sx={{ mt: 4, py: 2 }}>Submit</Button>
            <Typography sx={{ cursor: 'pointer', ":hover": { color: blue['200'] } }} variant='body1' onClick={() => nav('/login')}>Already have an account?</Typography>
        </Box>
    )
}
